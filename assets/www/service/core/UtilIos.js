...
com.swaas.hidoctor.edetailing.util.FileUtil = {
	    ...
	    getFileSize : function(fileName, onFileSizeGet){
	    	//var fileEntry=
            var _this = this;
            this.getFileEntry(fileName, function(fileEntry){
                //var file =
                _this.getFileFromFileEntry(fileEntry, function(file){
                    if(file != null){
                        var size=0;
                        size=(file.size/1024);
                        size = size/1024;
                        onFileSizeGet(size.toFixed(2));
                    }else{
                        onFileSizeGet(0);
                    }
                });
	    ...
	    deleteFile : function(fileName){
		 ...
		 checkIfFileExists: function(path, onFileFound){
			//alert(path);
			...
			try {
				window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, function(fileSystem){
					fileSystem.root.getFile(path, { create: false }, function(fileEntry){
						fileExists = true;
											//alert("fileExist->"+fileExists);
						onFileFound(fileExists);
					}, function (fileEntry){
						fileExists = false;
						onFileFound(fileExists);
					});
		 ...
		 getFileEntry : function(filePath, onFileFound, onFileNotFound){
		 ...
		 getFileFromFileEntry : function(fileEntry, onFileGet){
